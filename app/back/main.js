    ...
    await getChannelInfo("Mavisku")
      .then(async (channelInfo) => {
        ...
        socket.on("close", (code, reason) => {
          console.log(
            `WebSocket connection closed with code ${code}: ${reason}`
          );
        });
...
function createWindow() {
  const mainWindow = new BrowserWindow({
    webPreferences: {
      ...
      nodeIntegration: true, // Assurez-vous que nodeIntegration est activé
...
function handleWebSocketMessage(socket, message, streamerId) {
  switch (message.metadata.message_type) {
    case "session_welcome":
      ...
      createEventSubSubscriptionForFollowers(
        streamerId,
        message.payload.session.id
      );
      createEventSubSubscriptionForSubFisrtTime(
        streamerId,
        message.payload.session.id
      );
      createEventSubSubscriptionForSubGift(
        streamerId,
        message.payload.session.id
      );
      createEventSubSubscriptionForReSub(
        streamerId,
        message.payload.session.id
      );
      createEventSubSubscriptionForCheers(
        streamerId,
        message.payload.session.id
      );
      createEventSubSubscriptionForRaid(streamerId, message.payload.session.id);
      createEventSubSubscriptionForChannelPoints(
        streamerId,
        message.payload.session.id
      );
      break;
    ...
    case "notification":
      ...
      console.log(
        `Message type: ${message.metadata.message_type}, Notification type: ${message.payload.subscription.type}`
      );
...
async function lights(transitionType, timeOut) {
  const controller = new Control("192.168.1.13", { ack: Control.ackMask(0) });
  ...
  controller
    .setCustomPattern(my_effect, 100)
    .then((success) => {
